...
artifact; F-15, the Step 3.4 real-Pre-Trade-Risk scope record; F-16, the
Step 3.5 simulated-fill realism scope record; F-17, deferred position P&L
and the absence of an immutable event stream) that are tracked
defects/open gaps requiring resolution before
...
| F-17 | `realized_pnl_today`, `unrealized_pnl` (left at 0); no immutable event stream for state transitions | `core.positions`; order/position lifecycle (code-structural) | **Step 3.5 scope record (S-5), not a defect.** (1) **P&L deferred to Step 3.6:** the fill updates position `quantity`, `average_entry_price`, `market_value` (mark-to-market = quantity × fill price), and `last_price`/`last_price_at` (§10.6.6). It does **not** compute `realized_pnl_today` (on reducing/closing/crossing trades) or `unrealized_pnl` — those, plus P&L attribution, are the Step 3.6 P&L engine (§10.9.5); `apply_fill_to_position` deliberately maintains only position state (quantity + cost basis), leaving the P&L columns at their schema default 0. (2) **No immutable event stream:** §10.7.4 / §10.6.6 / §10.9.3 each require "every state transition shall produce an immutable event ... per P-5." Step 3.5 persists transitions as **row state** (`core.orders.status`, `core.positions` snapshot with `sequence_number`) plus the immutable `core.executions` fill row and the `analytics.risk_assessments` rejection audit — but there is **no dedicated event table / event-sourcing log / domain-event bus**, and `audit.audit_log` (Step 1.1 schema) is not wired for order/position transitions | (1) P&L before **Step 3.6** (its explicit deliverable). (2) The event stream before any consumer needs an ordered, replayable transition history (event-driven downstream services, full trade reconstruction per §329, or P-5 event-audit) — whichever comes first | (1) Compute realized/unrealized P&L in Step 3.6 against the position state established here. (2) Introduce an events table or wire `audit.audit_log` when a transition-event consumer exists | Step 3.5: `domain/portfolio/positions.py`, `application/execution/service.py`, `persistence/repositories/portfolio.py`; Doc 14 §10.6.6, §10.9.3, §10.9.5, §10.7.4; P-5 |
