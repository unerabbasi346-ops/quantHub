...
strategy versioning; extended in Phase 3 planning — F-11, an accepted
divergence record, not an open gap) that are tracked defects/open gaps
requiring resolution before a specific future phase/workflow begins —
unlike Phase 4/5 above, these are NOT accepted design decisions (F-11 is
the sole exception: it documents an already-resolved, deliberate
divergence, kept in this table for visibility alongside the related
F-2/F-3/F-4 entries it distinguishes itself from). Recorded here so each
