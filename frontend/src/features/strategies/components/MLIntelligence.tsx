...
import { Brain, Cpu, Shield, Target, TrendingDown, TrendingUp, Zap } from 'lucide-react'
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          {isLong ? <TrendingUp size={15} className="text-profit" /> : <TrendingDown size={15} className="text-risk" />}
          <Badge variant={isLong ? 'profit' : 'risk'}>{signal.direction}</Badge>
        </div>
        <span className="text-[11px] text-fg-subtle">{formatTimestamp(signal.ts)}</span>
      </div>

      {symbol && <div className="font-mono text-sm text-fg">{symbol}</div>}

      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <div className="text-[10px] uppercase tracking-wide text-fg-subtle">Signal strength</div>
          <div className={cn('font-mono font-semibold tabular-nums', value >= 0 ? 'text-profit' : 'text-risk')}>{formatSignalStrength(value)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wide text-fg-subtle">Implied size</div>
          <div className="font-mono font-semibold tabular-nums text-fg">
            {signal.implied_size_usdt != null ? formatCapital(Number.parseFloat(signal.implied_size_usdt)) : '—'}
          </div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wide text-fg-subtle">Leverage</div>
          <div className="font-mono font-semibold tabular-nums text-fg">{Number.parseFloat(signal.implied_leverage).toFixed(1)}x</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-wide text-fg-subtle">Valid</div>
          <div className="font-mono font-semibold text-fg">{signal.validation_status}</div>
        </div>
      </div>
      ...
      <div className="border-t border-border pt-3">
        {hasMl ? (
          ...
          <p className="text-[11px] text-fg-subtle">ML analysis not available — train a model first.</p>
        )}
      </div>
      ...
      {(tp != null || sl != null) && (
        <div className="space-y-1.5 border-t border-border pt-3 text-[11px]">
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1 text-fg-subtle"><Target size={11} /> ML-suggested TP</span>
            <span className="font-mono text-fg">{tp != null ? formatCapital(tp) : '—'}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1 text-fg-subtle"><Shield size={11} /> ML-suggested SL</span>
            <span className="font-mono text-fg">{sl != null ? formatCapital(sl) : '—'}</span>
          </div>
          {breakeven != null && (
            <div className="flex items-center justify-between">
              <span className="text-fg-subtle">Breakeven</span>
              <span className="font-mono text-fg">{formatCapital(breakeven)}</span>
            </div>
          )}
          <div className="flex items-center justify-between">
            <span className="text-fg-subtle">Risk/Reward</span>
            <span className="font-mono font-semibold text-fg">{rr != null ? `${rr.toFixed(1)}:1` : '—'}</span>
          </div>
        </div>
      )}
    </Card>
...
function ModelPerformancePanel({ models, isLoading }: { models: MLModelStatus[]; isLoading: boolean }) {
  ...
  return (
    <Panel className="space-y-3 p-5">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[11px] uppercase tracking-wide text-fg-subtle">Training accuracy</span>
        ...
        <span className="text-[10px] italic text-fg-subtle" title={ACCURACY_CAVEAT}>(training, not OOS)</span>
      </div>
      <p className="border-t border-border pt-3 text-[11px] leading-relaxed text-fg-subtle">
        Train on more data via the{' '}
        <Link href="/research" className="text-accent hover:underline">Research page</Link>{' '}
        for better signals.
      </p>
    </Panel>
  )
  ...
  if (!hmm) {
    return (
      <EmptyState
        ...
        description="Regime detection requires an HMMRegimeDetector model — train one via the Research page."
      />
    )
  }
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <div>
          <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-fg-subtle">Recent signals</h3>
